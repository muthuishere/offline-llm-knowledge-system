...
function re(o) {
  ...
  }, f = o[0] === S[0] && o[1] === S[1] && o[2] === S[2] && o[3] === S[3];
  if (e += 4, !f) throw new Error("Invalid magic number");
  if (t() !== 1) throw new Error("Invalid version number");
  const u = l(8), F = H[u];
  if (!F) throw new Error(`Unknown message name: ${u}`);
  const k = { _name: u };
  for (const p of F.fields) {
    const _ = t();
    if (_ === R) {
      if (!p.isNullable) throw new Error(`${u}: Expect field ${p.name} to be non-nullable`);
      k[p.name] = null;
      continue;
    }
    if (_ !== z[p.type]) throw new Error(`${u}: Expect field ${p.name} to have type ${p.type}`);
    k[p.name] = g(p);
  }
  return k;
}
function te(o) {
  const e = H[o._name];
  if (!e) throw new Error(`Unknown message name: ${o._name}`);
  const r = [], t = (d) => {
    const u = new ArrayBuffer(4);
    new DataView(u).setUint32(0, d, true), r.push(new Uint8Array(u));
  }, a = (d) => {
    const u = new ArrayBuffer(4);
    new DataView(u).setInt32(0, d, true), r.push(new Uint8Array(u));
  }, n = (d) => {
    const u = new ArrayBuffer(4);
    new DataView(u).setFloat32(0, d, true), r.push(new Uint8Array(u));
  }, s = (d) => {
    t(d ? 1 : 0);
  }, l = (d) => {
    const u = new TextEncoder().encode(d);
    t(u.byteLength), r.push(u);
  }, m = (d) => {
    t(d.byteLength), r.push(d);
  }, i = (d, u) => {
    t(d.length);
    for (const F of d) u(F);
  };
  r.push(S), t(1);
  {
    const d = new TextEncoder().encode(o._name);
    r.push(d);
  }
  for (const d of e.fields) {
    const u = o[d.name];
    if (!d.isNullable && u == null) throw new Error(`${o._name}: Expect field ${d.name} to be non-nullable`);
    if (u == null) {
      t(R);
      continue;
    }
    switch (t(z[d.type]), d.type) {
      case "str":
        l(u);
        break;
      case "int":
  ...
  const h = r.reduce((d, u) => d + u.byteLength, 0), g = new Uint8Array(h);
  let f = 0;
  for (const d of r) g.set(d, f), f += d.byteLength;
  return g;
}
const ae = (o) => {
  const e = o.reduce((t, a) => t + a.length, 0), r = new Uint8Array(e);
  r.set(o[0], 0);
  for (let t = 1; t < o.length; t++) r.set(o[t], o[t - 1].length);
  return r;
}, ne = new TextDecoder(), A = (o) => ne.decode(o), x = /-(\d{5})-of-(\d{5})\.gguf(?:\?.*)?$/, I = (o) => {
  const e = o.match(x);
  return e ? { baseURL: o.replace(x, ""), current: parseInt(e[1]), total: parseInt(e[2]) } : { baseURL: o, current: 1, total: 1 };
}, oe = (o) => {
  o.every((r) => !!r.name) && o.length > 1 && o.sort((t, a) => {
    const n = I(t.name), s = I(a.name);
    return n.current - s.current;
  });
}, C = (o) => new URL(o, document.baseURI).href, T = (o) => o.reduce((e, r) => e + r, 0), B = (o) => !!(o == null ? void 0 : o.startsWith), se = () => (async (o) => {
  try {
    return typeof MessageChannel < "u" && new MessageChannel().port1.postMessage(new SharedArrayBuffer(1)), WebAssembly.validate(o);
  } catch {
    return false;
  }
})(new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0, 1, 4, 1, 96, 0, 0, 3, 2, 1, 0, 5, 4, 1, 3, 1, 1, 10, 11, 1, 9, 0, 65, 0, 254, 16, 2, 0, 26, 11])), ie = async () => WebAssembly.validate(new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0, 1, 4, 1, 96, 0, 0, 3, 2, 1, 0, 10, 8, 1, 6, 0, 6, 64, 25, 11, 11])), le = async () => WebAssembly.validate(new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0, 1, 5, 1, 96, 0, 1, 123, 3, 2, 1, 0, 10, 10, 1, 8, 0, 65, 0, 253, 15, 253, 98, 11])), de = async () => {
  if (!await ie()) throw new Error("WebAssembly runtime does not support exception handling");
  if (!await le()) throw new Error("WebAssembly runtime does not support SIMD");
}, ue = /^.*\.gguf(?:\?.*)?$/, U = (o) => ue.test(o), W = () => !!navigator.userAgent.match(/Version\/([0-9\._]+).*Mobile.*Safari.*/), N = (o) => {
  const e = URL.createObjectURL(B(o) ? new Blob([o], { type: "text/javascript" }) : o);
  return new Worker(e, { type: "module" });
}, me = (o) => (...e) => {
  let r = [], t;
  return r.push(new Promise((a) => {
    t = a;
  })), o(...e, (a, n) => {
    t([a, n]), r.push(new Promise((s) => {
      t = s;
    }));
  }), (async function* () {
    let a;
    for (let n = 0, s = false; !s; n++) [a, s] = await r[n], delete r[n], a !== void 0 && (yield a);
  })();
}, ce = "b7179-4abef75", pe = `// Start the main llama.cpp
let wllamaMalloc;
let wllamaStart;
let wllamaAction;
let wllamaExit;
...
`);
  ...
  async fileWrite(e, r) {
    ...
    for (; ; ) {
      ...
      await this.pushTask({ verb: "fs.write", args: [e, s, a], callbackId: this.taskId++ }, [s.buffer]), a += l;
    }
  }
  parseResult(e) {
    const r = JSON.parse(e);
    if (r && r.error) throw new Error("Unknown error, please see console.log");
    return r;
  }
  pushTask(e, r) {
    return new Promise((t, a) => {
      this.taskQueue.push({ resolve: t, reject: a, param: e, buffers: r }), this.runTaskLoop();
    });
  }
  async runTaskLoop() {
    if (!this.busy) {
      for (this.busy = true; ; ) {
        ...
        if (!e) break;
        this.resultQueue.push(e), this.worker.postMessage(e.param, W() ? void 0 : { transfer: e.buffers ?? [] });
      }
      this.busy = false;
    }
  }
  onRecvMsg(e) {
    ...
    if (r && r.startsWith("console.")) {
      ...
      r.endsWith("debug") && this.logger.debug(...t), r.endsWith("log") && this.logger.log(...t), r.endsWith("warn") && this.logger.warn(...t), r.endsWith("error") && this.logger.error(...t);
      return;
    } else r === "signal.abort" && this.abort(t[0]);
    const { callbackId: a, result: n, err: s } = e.data;
    if (a) {
      const l = this.resultQueue.findIndex((m) => m.param.callbackId === a);
      if (l !== -1) {
        const m = this.resultQueue.splice(l, 1)[0];
        s ? m.reject(s) : m.resolve(n);
      } else this.logger.error(`Cannot find waiting task with callbackId = ${a}`);
    }
  }
  abort(e) {
    for (; this.resultQueue.length > 0; ) {
      const r = this.resultQueue.pop();
      if (!r) break;
      r.reject(new Error(`Received abort signal from llama.cpp; Message: ${e || "(empty)"}`));
    }
  }
}
const w = "__metadata__", ge = "polyfill_for_older_version";
class j {
  async getNameFromURL(e) {
    return await v(e, "");
  }
  async write(e, r, t) {
    return this.writeMetadata(e, t), await O(e, r);
  }
  async download(e, r = {}) {
    const t = N(G);
    let a = false;
    r.signal && (a = r.signal.aborted, r.signal.addEventListener("abort", () => {
      a = true, t.postMessage({ action: "download-abort" });
    }), delete r.signal);
    const n = await v(e, w), s = await v(e, "");
    return await new Promise((l, m) => {
      t.postMessage({ action: "download", url: e, filename: s, metadataFileName: n, options: { headers: r.headers, aborted: a } }), t.onmessage = (i) => {
        var _a;
        if (i.data.ok) t.terminate(), l();
        else if (i.data.err) t.terminate(), m(i.data.err);
        else if (i.data.progress) {
          const h = i.data.progress;
          (_a = r.progressCallback) == null ? void 0 : _a.call(r, h);
        } else m(new Error("Unknown message from worker")), console.error("Unknown message from worker", i.data);
      };
    });
  }
  async open(e) {
  ...
  async getMetadata(e) {
    const r = await D(e, w), t = await this.getSize(e);
    if (!r) return t > 0 ? { etag: ge, originalSize: t, originalURL: "" } : null;
    try {
      return await new Response(r).json();
    } catch {
      return null;
    }
  }
  async list() {
    const e = await b(), r = [], t = {};
    for await (let [a, n] of e.entries()) if (n.kind === "file" && a.startsWith(w)) {
      const s = (await n.getFile()).stream(), l = await new Response(s).json().catch((m) => null);
      t[a.replace(w, "")] = l;
    }
    for await (let [a, n] of e.entries()) n.kind === "file" && !a.startsWith(w) && r.push({ name: a, size: await n.getFile().then((s) => s.size), metadata: t[a] || { originalSize: (await n.getFile()).size, originalURL: "", etag: "" } });
    return r;
  }
  async clear() {
  ...
  async delete(e) {
    const r = await this.getNameFromURL(e);
    await this.deleteMany((t) => t.name === e || t.name === r);
  }
  async deleteMany(e) {
    const r = await b(), t = await this.list();
    for (const a of t) e(a) && r.removeEntry(a.name);
  }
  async writeMetadata(e, r) {
    const t = new Blob([JSON.stringify(r)], { type: "text/plain" });
    await O(e, t.stream(), w);
  }
}
async function O(o, e, r = "") {
  try {
    const t = await v(o, r), a = await ve(t);
    await a.truncate(0);
    const n = e.getReader();
    for (; ; ) {
    ...
    console.error("opfsWrite", t);
  }
}
async function D(o, e = "") {
  const r = async (n) => {
    try {
      return await (await (await b()).getFileHandle(n)).getFile();
    } catch {
      return null;
    }
  };
  ...
  if (t) return t;
  const a = await v(o, e);
  return t = await r(a), t;
}
async function we(o, e = "") {
  try {
    const r = await b(), t = await v(o, e);
    return (await (await r.getFileHandle(t)).getFile()).size;
  } catch {
    return -1;
  }
}
async function v(o, e) {
  const r = await crypto.subtle.digest("SHA-1", new TextEncoder().encode(o)), a = Array.from(new Uint8Array(r)).map((n) => n.toString(16).padStart(2, "0")).join("");
  return `${e}${a}_${o.split("/").pop()}`;
}
async function b() {
  return await (await navigator.storage.getDirectory()).getDirectoryHandle("cache", { create: true });
}
async function ve(o) {
  const e = N(G);
  let r, t;
  e.onmessage = (n) => {
    n.data.ok ? r(null) : n.data.err && t(n.data.err);
  };
  const a = (n) => new Promise((s, l) => {
    r = s, t = l, e.postMessage(n, W() ? void 0 : { transfer: n.value ? [n.value.buffer] : [] });
  });
  return await a({ open: o }), { truncate: async () => {
  }, write: (n) => a({ value: n }), close: async () => {
    await a({ done: true }), e.terminate();
  } };
}
const ye = 3;
var Se = ((o) => (o.VALID = "valid", o.INVALID = "invalid", o.DELETED = "deleted", o))(Se || {});
class P {
  constructor(e, r, t) {
    this.modelManager = e, this.url = r, t ? (this.files = this.getAllFiles(t), this.size = T(this.files.map((a) => a.metadata.originalSize))) : (this.files = [], this.size = 0);
  }
  async open() {
    if (this.size === -1) throw new c("Model is deleted from the cache; Call ModelManager.downloadModel to re-download the model", "load_error");
    const e = [];
    for (const r of this.files) {
      const t = await this.modelManager.cacheManager.open(r.name);
      if (!t) throw new Error(`Failed to open file ${r.name}; Hint: the model may be invalid, please refresh it`);
      e.push(t);
    }
    return e;
  }
  validate() {
    const e = y.parseModelUrl(this.url).length;
    if (this.size === -1) return "deleted";
    if (this.size < 16 || this.files.length !== e) return "invalid";
    for (const r of this.files) if (!r.metadata || r.metadata.originalSize !== r.size) return "invalid";
    return "valid";
  }
  async refresh(e = {}) {
    const r = y.parseModelUrl(this.url), t = r.map((i, h) => ({ url: i, index: h }));
    this.modelManager.logger.debug("Downloading model files:", r);
    const a = this.modelManager.params.parallelDownloads ?? ye, n = await this.getTotalDownloadSize(r), s = [], l = async () => {
      for (; t.length > 0; ) {
        const i = t.shift();
        if (!i) break;
        await this.modelManager.cacheManager.download(i.url, { ...e, progressCallback: ({ loaded: h }) => {
          var _a;
          s[i.index] = h, (_a = e.progressCallback) == null ? void 0 : _a.call(e, { loaded: T(s), total: n });
        } });
      }
    }, m = [];
    for (let i = 0; i < a; i++) m.push(l()), s.push(0);
    await Promise.all(m), this.files = this.getAllFiles(await this.modelManager.cacheManager.list()), this.size = this.files.reduce((i, h) => i + h.metadata.originalSize, 0);
  }
  async remove() {
    this.files = this.getAllFiles(await this.modelManager.cacheManager.list()), await this.modelManager.cacheManager.deleteMany((e) => !!this.files.find((r) => r.name === e.name)), this.size = -1;
  }
  getAllFiles(e) {
    const r = new Set(y.parseModelUrl(this.url)), t = [];
    for (const a of r) {
      const n = e.find((s) => s.metadata.originalURL === a);
      if (!n) throw new Error(`Model file not found: ${a}`);
      t.push(n);
    }
    return t.sort((a, n) => a.metadata.originalURL.localeCompare(n.metadata.originalURL)), t;
  }
  async getTotalDownloadSize(e) {
    const t = (await Promise.all(e.map((a) => fetch(a, { method: "HEAD" })))).map((a) => Number(a.headers.get("content-length") || "0"));
    return T(t);
  }
}
class y {
  constructor(e = {}) {
    this.cacheManager = e.cacheManager || new j(), this.params = e, this.logger = e.logger || console;
  }
  static parseModelUrl(e) {
    var _a;
    if (Array.isArray(e)) return e;
    const r = /-(\d{5})-of-(\d{5})\.gguf(?:\?.*)?$/, a = ((_a = e.match(/\.gguf(\?.*)?$/)) == null ? void 0 : _a[1]) ?? "", n = e.match(r);
    if (!n) return [e];
    const s = e.replace(r, ""), l = n[2];
    return Array.from({ length: Number(l) }, (i, h) => (h + 1).toString().padStart(5, "0")).map((i) => `${s}-${i}-of-${l}.gguf${a}`);
  }
  async getModels(e = {}) {
    const r = await this.cacheManager.list();
    let t = [];
    for (const a of r) {
      const n = y.parseModelUrl(a.metadata.originalURL);
      (n.length === 1 || n[0] === a.metadata.originalURL) && t.push(new P(this, a.metadata.originalURL, r));
    }
    return e.includeInvalid || (t = t.filter((a) => a.validate() === "valid")), t;
  }
  async downloadModel(e, r = {}) {
    if (!U(e)) throw new c(`Invalid model URL: ${e}; URL must ends with ".gguf"`, "download_error");
    const t = new P(this, e, void 0);
    return t.validate() !== "valid" && await t.refresh(r), t;
  }
  async getModelOrDownload(e, r = {}) {
    var _a;
    const a = (await this.getModels()).find((n) => n.url === e);
    return a ? ((_a = r.progressCallback) == null ? void 0 : _a.call(r, { loaded: a.size, total: a.size }), a) : this.downloadModel(e, r);
  }
  async clear() {
    await this.cacheManager.clear();
  }
}
const Fe = /^([a-zA-Z0-9_\-\.]+)\/([a-zA-Z0-9_\-\.]+)$/, Ee = "Hugging Face model ID is incorrect. Only regular alphanumeric characters, '-', '.' and '_' supported", be = { ...console, debug: () => {
} };
...
class L extends Error {
  constructor() {
    super("Operation aborted"), this.name = "AbortError";
  }
}
class ke {
  constructor(e, r = {}) {
    if (this.proxy = null, this.useMultiThread = false, this.nbThreads = 1, this.useEmbeddings = false, this.loadedContextInfo = null, this.bosToken = -1, this.eosToken = -1, this.eotToken = -1, this.eogTokens = /* @__PURE__ */ new Set(), this.addBosToken = false, this.addEosToken = false, this.samplingConfig = {}, this.hasEncoder = false, this.decoderStartToken = -1, this.nCachedTokens = 0, de(), !e) throw new c("AssetsPathConfig is required");
    this.pathConfig = e, this.config = r, this.cacheManager = r.cacheManager ?? new j(), this.modelManager = r.modelManager ?? new y({ cacheManager: this.cacheManager, logger: r.logger ?? console, parallelDownloads: r.parallelDownloads, allowOffline: r.allowOffline });
  }
  logger() {
    return this.config.logger ?? console;
  }
  checkModelLoaded() {
    if (!this.isModelLoaded()) throw new c("loadModel() is not yet called", "model_not_loaded");
  }
  ...
  isTokenEOG(e) {
    return e === this.eosToken || e === this.eotToken || this.eogTokens.has(e);
  }
  ...
  getChatTemplate() {
    return this.checkModelLoaded(), this.chatTemplate ?? null;
  }
  async loadModelFromUrl(e, r = {}) {
    const t = B(e) ? e : e[0], s = await (r.useCache ?? true ? await this.modelManager.getModelOrDownload(t, r) : await this.modelManager.downloadModel(t, r)).open();
    return await this.loadModel(s, r);
  }
  async loadModelFromHF(e, r, t = {}) {
    if (!e.match(Fe)) throw new c(Ee, "download_error");
    if (!U(r)) throw new c("Only GGUF file is supported", "download_error");
    return await this.loadModelFromUrl(`https://huggingface.co/${e}/resolve/main/${r}`, t);
  }
  async loadModel(e, r = {}) {
    const t = e instanceof P ? await e.open() : [...e];
    if (t.some((d) => d.size === 0)) throw new c("Input model (or splits) must be non-empty Blob or File", "load_error");
    if (oe(t), this.proxy) throw new c("Module is already initialized", "load_error");
    const a = await se();
    a || this.logger().warn("Multi-threads are not supported in this environment, falling back to single-thread");
    const n = !!this.pathConfig["multi-thread/wllama.wasm"];
    n || this.logger().warn('Missing paths to "multi-thread/wllama.wasm", falling back to single-thread');
    const s = Math.floor((navigator.hardwareConcurrency || 1) / 2), l = r.n_threads ?? s;
    this.nbThreads = l, this.useMultiThread = a && n && l > 1;
    const m = this.useMultiThread ? { "wllama.wasm": C(this.pathConfig["multi-thread/wllama.wasm"]) } : { "wllama.wasm": C(this.pathConfig["single-thread/wllama.wasm"]) };
    this.proxy = new _e(m, this.useMultiThread ? l : 1, this.config.suppressNativeLog ?? false, this.logger());
    const i = t.map((d, u) => ({ name: `model-${u}.gguf`, blob: d }));
    await this.proxy.moduleInit(i);
    const h = await this.proxy.wllamaStart();
    if (!h.success) throw new c(`Error while calling start function, result = ${h}`);
    const g = await this.proxy.wllamaAction("load", { _name: "load_req", use_mmap: true, use_mlock: true, n_gpu_layers: 0, seed: r.seed || Math.floor(Math.random() * 1e5), n_ctx: r.n_ctx || 1024, n_threads: this.useMultiThread ? l : 1, n_ctx_auto: false, model_paths: i.map((d) => `models/${d.name}`), embeddings: r.embeddings, offload_kqv: r.offload_kqv, n_batch: r.n_batch, pooling_type: r.pooling_type, rope_scaling_type: r.rope_scaling_type, rope_freq_base: r.rope_freq_base, rope_freq_scale: r.rope_freq_scale, yarn_ext_factor: r.yarn_ext_factor, yarn_attn_factor: r.yarn_attn_factor, yarn_beta_fast: r.yarn_beta_fast, yarn_beta_slow: r.yarn_beta_slow, yarn_orig_ctx: r.yarn_orig_ctx, cache_type_k: r.cache_type_k, cache_type_v: r.cache_type_v, n_seq_max: 1, flash_attn: r.flash_attn, swa_full: true }), f = { ...g, metadata: {} };
    for (let d = 0; d < g.metadata_key.length; d++) f.metadata[g.metadata_key[d]] = g.metadata_val[d];
    this.bosToken = f.token_bos, this.eosToken = f.token_eos, this.eotToken = f.token_eot, this.useEmbeddings = !!r.embeddings, this.metadata = { hparams: { nVocab: f.n_vocab, nCtxTrain: f.n_ctx_train, nEmbd: f.n_embd, nLayer: f.n_layer }, meta: f.metadata }, this.hasEncoder = !!f.has_encoder, this.decoderStartToken = f.token_decoder_start, this.addBosToken = f.add_bos_token, this.addEosToken = f.add_eos_token, this.chatTemplate = f.metadata["tokenizer.chat_template"], this.loadedContextInfo = f, this.eogTokens = new Set(f.list_tokens_eog), this.logger().debug({ loadedCtxInfo: f });
  }
  getLoadedContextInfo() {
    if (this.checkModelLoaded(), !this.loadedContextInfo) throw new c("Loaded context info is not available");
    return { ...this.loadedContextInfo };
  }
  async createEmbedding(e, r = {}) {
    this.checkModelLoaded();
    const t = { skipBOS: false, skipEOS: false, ...r };
    await this.samplingInit(this.samplingConfig), await this.kvClear();
    const a = await this.tokenize(e);
    return this.bosToken && !t.skipBOS && a.unshift(this.bosToken), this.eosToken && !t.skipEOS && a.push(this.eosToken), await this.embeddings(a);
  }
  async createChatCompletion(e, r) {
    const t = await this.formatChat(e, true);
    return r.stream ? await this.createCompletionGenerator(t, r) : await this.createCompletion(t, { ...r, stream: false });
  }
  async createCompletion(e, r) {
    return r.stream ? await this.createCompletionGenerator(e, r) : await this.createCompletionImpl(e, { ...r, stream: false });
  }
  async createCompletionImpl(e, r) {
    var _a;
    this.checkModelLoaded(), this.samplingConfig = r.sampling ?? {}, await this.samplingInit(this.samplingConfig);
    const t = new Set(r.stopTokens ?? []);
    let a = await this.tokenize(e, true);
    this.addBosToken && a[0] !== this.bosToken && a.unshift(this.bosToken), r.useCache ? a = await this.computeNonCachedTokens(a) : await this.kvClear(), await this.samplingAccept(a), this.isEncoderDecoderArchitecture() ? (await this.encode(a), await this.decode([this.getDecoderStartToken()], {})) : await this.decode(a, {});
    let n = new Uint8Array(), s = false;
    const l = () => {
      s = true;
    };
    for (let m = 0; m < (r.nPredict ?? 1 / 0); m++) {
      const i = await this.samplingSample();
      if (this.isTokenEOG(i.token) || t.has(i.token) || (n = ae([n, i.piece]), r.onNewToken && r.onNewToken(i.token, i.piece, A(n), { abortSignal: l }), s || ((_a = r.abortSignal) == null ? void 0 : _a.aborted))) break;
      await this.samplingAccept([i.token]), await this.decode([i.token], {});
    }
    return A(n);
  }
  createCompletionGenerator(e, r) {
    return new Promise((t, a) => {
      const n = me((s) => {
        this.createCompletionImpl(e, { ...r, onNewToken: (l, m, i) => {
          s({ token: l, piece: m, currentText: i }, false);
        } }).catch(a).then(() => {
          s(void 0, true);
        });
      });
  ...
  async samplingInit(e, r = []) {
    this.checkModelLoaded(), this.samplingConfig = e;
    const t = e.logit_bias ?? [], a = t.map((l) => l.token), n = t.map((l) => l.bias);
    if (!(await this.proxy.wllamaAction("sampling_init", { _name: "sint_req", ...e, logit_bias_toks: a, logit_bias_vals: n, tokens: r })).success) throw new c("Failed to initialize sampling");
  }
  async getVocab() {
    return this.checkModelLoaded(), (await this.proxy.wllamaAction("get_vocab", { _name: "gvoc_req" })).vocab;
  }
  async lookupToken(e) {
    this.checkModelLoaded();
    const r = await this.proxy.wllamaAction("lookup_token", { _name: "lkup_req", piece: e });
    return r.success ? r.token : -1;
  }
  async tokenize(e, r = true) {
    return this.checkModelLoaded(), (await this.proxy.wllamaAction("tokenize", { _name: "tokn_req", text: e, special: !!r })).tokens;
  }
  async detokenize(e, r = false) {
    this.checkModelLoaded();
    const t = await this.proxy.wllamaAction("detokenize", { _name: "dtkn_req", tokens: e });
    return r ? A(t.buffer) : t.buffer;
  }
  async decode(e, r) {
    var _a;
    if (this.checkModelLoaded(), this.useEmbeddings) throw new c("embeddings is enabled. Use wllama.setOptions({ embeddings: false }) to disable it.");
    if (e.length === 0) return { nPast: this.nCachedTokens };
    if (this.nCachedTokens + e.length > this.loadedContextInfo.n_ctx) throw new c("Running out of context cache. Please increase n_ctx when loading the model", "kv_cache_full");
    const t = this.breakTokensIntoBatches(e, this.loadedContextInfo.n_batch);
    let a;
    for (let n = 0; n < t.length; n++) {
      if ((_a = r == null ? void 0 : r.abortSignal) == null ? void 0 : _a.aborted) throw new L();
      const s = t.length > 1 && n < t.length - 1;
      if (a = await this.proxy.wllamaAction("decode", { _name: "deco_req", tokens: t[n], skip_logits: r.skipLogits || s }), a.error) throw new c(a.error);
      if (!a.success) throw new c("Cannot encode, unknown error");
    }
    return this.nCachedTokens = a.n_past, { nPast: a.n_past };
  }
  async encode(e, r) {
    var _a;
    if (this.checkModelLoaded(), !this.hasEncoder) throw new c("This model does not use encoder-decoder architecture.", "inference_error");
    if (this.useEmbeddings) throw new c("embeddings is enabled. Use wllama.setOptions({ embeddings: false }) to disable it.", "inference_error");
    if (e.length === 0) return { nPast: this.nCachedTokens };
    if (this.nCachedTokens + e.length > this.loadedContextInfo.n_ctx) throw new c("Running out of context cache. Please increase n_ctx when loading the model", "kv_cache_full");
    const t = this.breakTokensIntoBatches(e, this.loadedContextInfo.n_batch);
    let a;
    for (let n = 0; n < t.length; n++) {
      if ((_a = r == null ? void 0 : r.abortSignal) == null ? void 0 : _a.aborted) throw new L();
      if (a = await this.proxy.wllamaAction("encode", { _name: "enco_req", tokens: t[n] }), a.error) throw new c(a.error);
      if (!a.success) throw new c("Cannot encode, unknown error");
    }
    return this.nCachedTokens = a.n_past, { nPast: a.n_past };
  }
  breakTokensIntoBatches(e, r) {
    const t = [];
    for (let a = 0; a < e.length; a += r) t.push(e.slice(a, a + r));
    return t;
  }
  async samplingSample() {
    this.checkModelLoaded();
    const e = await this.proxy.wllamaAction("sampling_sample", { _name: "ssam_req" });
    return { piece: e.piece, token: e.token };
  }
  async samplingAccept(e) {
    if (this.checkModelLoaded(), !(await this.proxy.wllamaAction("sampling_accept", { _name: "sacc_req", tokens: e })).success) throw new c("samplingAccept unknown error");
  }
  async getLogits(e = 40) {
    this.checkModelLoaded();
    const r = await this.proxy.wllamaAction("get_logits", { _name: "glog_req", top_k: e }), t = [];
    for (let a = 0; a < r.tokens.length; a++) t.push({ token: r.tokens[a], p: r.probs[a] });
    return t;
  }
  async embeddings(e) {
    if (this.checkModelLoaded(), !this.useEmbeddings) throw new c("embeddings is disabled. Use wllama.setOptions({ embeddings: true }) to enable it.", "inference_error");
    if (this.nCachedTokens > 0 && this.logger().warn("Embeddings: KV cache is not empty, this may produce incorrect results"), this.nCachedTokens + e.length > this.loadedContextInfo.n_ctx) throw new c("Running out of context cache. Please increase n_ctx when loading the model", "kv_cache_full");
    if (e.length > this.loadedContextInfo.n_batch) throw new c("Embedding tokens does not fit into batch. Please increase n_batch when loading the model", "inference_error");
    if (e.length > this.loadedContextInfo.n_ubatch) throw new c("Embedding tokens does not fit into physical batch. Please increase n_ubatch when loading the model", "inference_error");
    const r = await this.proxy.wllamaAction("embeddings", { _name: "gemb_req", tokens: e });
    if (r.success) return r.embeddings;
    throw new c("embeddings unknown error");
  }
  async kvRemove(e, r) {
    if (this.checkModelLoaded(), r === 0) return;
    if (!(await this.proxy.wllamaAction("kv_remove", { _name: "kvcr_req", n_keep: e, n_discard: r })).success) throw new c("kvRemove unknown error");
    r < 0 ? this.nCachedTokens = e : this.nCachedTokens -= r;
  }
  async kvClear() {
    if (this.checkModelLoaded(), !(await this.proxy.wllamaAction("kv_clear", { _name: "kvcc_req" })).success) throw new c("kvClear unknown error");
    this.nCachedTokens = 0;
  }
  async formatChat(e, r, t) {
    this.checkModelLoaded();
    const a = e.map((l) => l.role), n = e.map((l) => l.content), s = await this.proxy.wllamaAction("chat_format", { _name: "cfmt_req", roles: a, contents: n, tmpl: t, add_ass: r });
    if (!s.success) throw new c("formatChat unknown error");
    return s.formatted_chat;
  }
  async setOptions(e) {
    this.checkModelLoaded(), await this.proxy.wllamaAction("set_options", { _name: "opti_req", ...e }), this.useEmbeddings = e.embeddings;
  }
  async exit() {
    var _a;
    await ((_a = this.proxy) == null ? void 0 : _a.wllamaExit()), this.proxy = null;
  }
  async _getDebugInfo() {
    return this.checkModelLoaded(), await this.proxy.wllamaDebug();
  }
  async _testBenchmark(e, r) {
    return this.checkModelLoaded(), await this.proxy.wllamaAction("test_benchmark", { _name: "tben_req", type: e, n_samples: r });
  }
  async _testPerplexity(e) {
    return this.checkModelLoaded(), await this.proxy.wllamaAction("test_perplexity", { _name: "tper_req", tokens: e });
  }
  async getCachedTokens() {
    return this.checkModelLoaded(), (await this.proxy.wllamaAction("current_status", { _name: "stat_req" })).tokens;
  }
  async computeNonCachedTokens(e) {
    const r = await this.getCachedTokens();
    let t = 0;
    for (; t < Math.min(r.length, e.length) && r[t] === e[t]; t++) ;
    this.logger().debug(`Cache nKeep=${t}`);
    try {
      return await this.kvRemove(t, -1), e.slice(t, e.length);
    } catch {
      return this.logger().warn("Failed to rollback KV cache, clearing it instead"), await this.kvClear(), e;
    }
  }
}
export {
  be as LoggerWithoutDebug,
  P as Model,
  y as ModelManager,
  Se as ModelValidationStatus,
  ge as POLYFILL_ETAG,
  ke as Wllama,
  L as WllamaAbortError,
  c as WllamaError,
  U as isValidGgufFile
};
